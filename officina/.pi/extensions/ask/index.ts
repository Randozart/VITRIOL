// ask — vendored 2026-08-31 from @earendil-works/pi-coding-agent 0.83.0
// examples/extensions/question.ts (Apache-2.0), verbatim except this header.
// Tool name "question": the agent asks the user a clarifying question with
// pick-from-options + free-text escape hatch, mid-turn, answer returns as the
// tool result. Essential for small models that would otherwise guess.
// NOTICE: Apache-2.0 attribution chain in .pi/extensions/NOTICE.
...
export default function question(pi: ExtensionAPI) {
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			if (ctx.mode !== "tui") {
				return {
					...
					details: {
						...
						options: params.options.map((o: { label: string }) => o.label),
						answer: null,
					} as QuestionDetails,
				};
			}
			...
			const simpleOptions = params.options.map((o: { label: string }) => o.label);
