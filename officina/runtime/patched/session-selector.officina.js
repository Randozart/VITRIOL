// [officina] PATCHED VENDOR COPY.
// Source: @earendil-works/pi-coding-agent 0.83.0 (MIT)
//         dist/modes/interactive/components/session-selector.js
// Modification (2026-08-31, owner report): honest loading state — the empty
// list message claimed "No sessions in current folder" while the async scan
// was still running. SessionList gains a loading flag set by loadScope;
// while loading, render shows "Loading sessions…" instead of the misleading
// placeholder. Upstream MIT license terms preserved; see LICENSE in the
// package and the parent repo's LICENSE/LICENSE-MIT.
...
class SessionList {
    ...
    searchInput;
    showCwd = false;
    loading = false; // [officina] true while the parent is scanning the sessions dir
    sortMode = "threaded";
    // [officina] loading flag setter - parent marks the scan start/end so the
    // empty state can stay honest (see render).
    setLoading(loading) {
        this.loading = loading;
    }
    nameFilter = "all";
    ...
    render(width) {
        ...
        if (this.loading) {
            // [officina] honest placeholder during the async scan - the static
            // "No sessions in current folder" text read as if existing
            // sessions had vanished (owner report 2026-08-31).
            lines.push(theme.fg("muted", truncateToWidth("  Loading sessions…", width, "…")));
            return lines;
        }
...
export class SessionSelectorComponent extends Container {
    ...
    async loadScope(scope, reason) {
        ...
        this.sessionList.setLoading(true); // [officina]
        this.requestRender();
        ...
        try {
            ...
            this.sessionList.setLoading(false); // [officina]
            this.sessionList.setSessions(sessions, showCwd);
            this.requestRender();
        }
        catch (err) {
