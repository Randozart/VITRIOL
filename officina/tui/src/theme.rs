...
//!
//! Ported from vitriol-tui/src/theme.rs (canonical Vitriolum definition,
//! this repo, Apache-2.0). Palette derived from the Alka Officina dark
//! theme re-centered on the VITRIOL logo colours. Semantics map to
//! alchemy: solvent cyan = the process flowing; gold = work completed;
//! green = the living fire (healthy); red = substrate (crash).

use ratatui::style::{Color, Modifier, Style};

/// Background (GitHub-dark base).
...
/// VITRIOL primary green (Safety) — borders, headers, gauges.
pub const GREEN: Color = Color::Rgb(0x3F, 0xB9, 0x50);
/// VITRIOL gold (Sovereignty) — titles, active accents.
pub const GOLD: Color = Color::Rgb(0xFF, 0xD7, 0x00);
/// Solvent cyan — active/streaming decode.
pub const CYAN: Color = Color::Rgb(0x00, 0xFF, 0xFF);
/// Antidote orange — warnings.
pub const ORANGE: Color = Color::Rgb(0xFF, 0x5F, 0x1F);
/// Substrate red — down / critical.
...
/// Violet — files, custom labels (Vitriolum, from the JS palette).
pub const VIOLET: Color = Color::Rgb(0xB2, 0x94, 0xBB);
/// Cold blue — the heat ramp's low end; the officina divider.
pub const COLD_BLUE: Color = Color::Rgb(0x2E, 0x5F, 0xA3);
/// Light yellow — capacity ramp start.
pub const LIGHT_YELLOW: Color = Color::Rgb(0xFF, 0xE0, 0x66);
/// Deep red — capacity ramp end.
...
// ── Alchemical glyphs (verified: Unicode UCD nameslist, U+1F700 block) ──
// Font coverage is spotty beyond the four elements; fallbacks are one-line
// swaps here. See .opencode/plans/officina-tui-phase3-vitriolum-2026-09-02.md

/// The brand: ALCHEMICAL SYMBOL FOR VITRIOL (U+1F716). Fallback: ◈
pub const GLYPH_VITRIOL: &str = "🜖";
/// The living process: FIRE (U+1F702) — proven coverage (vitriol-tui).
pub const GLYPH_FIRE: &str = "🜂";
/// Compaction/distillation: ALEMBIC U+2697 (Misc Symbols, wide support).
...
pub const GLYPH_QUINTESSENCE: &str = "🜀";
/// Reserved: PHILOSOPHERS SULFUR (U+1F70E) — coupling mark, when wired.
pub const GLYPH_PHIL_SULFUR: &str = "🜎";
/// Reserved: NIGHT (U+1F76F) — engine idle, when wired.
pub const GLYPH_NIGHT: &str = "🝯";
/// Reserved: HOUR (U+1F76E) — latency displays, when wired.
pub const GLYPH_HOUR: &str = "🝮";
/// Reserved: SALT (U+1F714) — files label, when wired.
pub const GLYPH_SALT: &str = "🜔";
/// Reserved: DISSOLVE (U+1F761) — abort semantics (footer hint text only).
pub const GLYPH_DISSOLVE: &str = "🝡";

// ── Elemental glyphs (proven coverage via vitriol-tui) ──────────────────
pub const GLYPH_GEN: &str = "🜂"; // fire, the forge
pub const GLYPH_HERM: &str = "🜄"; // water, the sealed flask of memory
pub const GLYPH_EMBED: &str = "🜁"; // air, spirit
pub const GLYPH_GPU: &str = "🜃"; // earth, silicon matter
...
/// Style for the top-level banner: gold, bold.
pub fn banner() -> Style {
    Style::new().fg(GOLD).bg(BG).add_modifier(Modifier::BOLD)
}

/// Muted gold for non-port (logical-layer) accents.
pub fn gold_muted() -> Style {
    Style::new().fg(GOLD).bg(BG)
}

/// Border style for a service panel, coloured by liveness.
...
pub enum BrailleRamp {
    ...
    Activity,
    /// Temperature: cold blue → cyan → gold → orange → red.
    Heat,
...
impl BrailleRamp {
    ...
    /// True when the ramp's high end is already a danger color.
...
/// Piecewise lerp across `stops` by fraction `t` in `[0, 1]`.
...
/// Value-text style for a gauge: bold orange past the warn threshold unless
/// the ramp already signals danger at its high end.
