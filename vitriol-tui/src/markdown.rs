...
//!
//! Renders markdown (via `pulldown-cmark`) into a `Vec<Line<'static>>` suitable
//! for the GUIDE reader: headings, bold/emphasis, inline + fenced code, lists,
//! blockquotes, rules, links, and pipe tables. Paragraphs are hard-wrapped to
//! `width` so the reader's scroll offset stays in logical-line units.
//!
